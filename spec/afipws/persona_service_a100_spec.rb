    ...
    context 'métodos API' do
      ...
      it 'jurisdictions' do
        savon.expects(:get_parameter_collection_by_name)
          .with(message: auth.merge(collectionName: 'SUPA.E_PROVINCIA'))
      ...
      it 'company_types' do
        savon.expects(:get_parameter_collection_by_name)
          .with(message: auth.merge(collectionName: 'SUPA.TIPO_EMPRESA_JURIDICA'))
      ...
      it 'public_organisms' do
        savon.expects(:get_parameter_collection_by_name)
          .with(message: auth.merge(collectionName: 'SUPA.E_ORGANISMO_INFORMANTE'))
